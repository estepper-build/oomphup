...
import static org.jboss.apiviz.Constant.ITALIC_FONT;
import static org.jboss.apiviz.Constant.NEWLINE;
import static org.jboss.apiviz.Constant.NORMAL_FONT;
import static org.jboss.apiviz.Constant.OPTION_CATEGORY;
import static org.jboss.apiviz.Constant.OPTION_NO_SUBTYPES;
 ...
 *
 * @version $Rev: 85 $, $Date: 2010-03-04 09:51:34 +0100 (Do, 04 Mrz 2010) $
 *
 */
public class ClassDocGraph {
    ...
    public ClassDocGraph(RootDoc root) {
        ...
        for (ClassDoc node: root.classes()) {
            PackageDoc packageDoc = node.containingPackage();
            packages.add(packageDoc);
            
            // Log a warning about missing class description that is clickable in Eclipse 
            if (!hasFirstSentence(node)) {
              warnings.add("Class description missing: " 
                  + packageDoc.name() + "." + node.typeName().replace('.', '$')
                  + ".class(" + node.position().file().getName() + ":" + node.position().line() + ")");
            }
        ...
        for (PackageDoc packageDoc : packages) {
          // Log a warning about missing package description
          if (!hasFirstSentence(packageDoc)) {
            warnings.add("Package description missing: " + packageDoc.name());
          }
        }
        
       List<String> list = new ArrayList<String>(warnings);
       Collections.sort(list);
       
       File warningsFile = warningsFile(root.options());
       if (warningsFile != null) {
         ...
         if (!list.isEmpty()) {
           ...
           try {
            stream = new PrintStream(warningsFile);
            for (String warning : list) {
    ...
    private boolean hasFirstSentence(Doc doc) {
      Tag[] firstSentence = doc.firstSentenceTags();
      return firstSentence != null && firstSentence.length != 0;
    }

    private static File warningsFile(String[][] options) {
      for (String[] o: options) {
        if (o[0].equals(Constant.OPTION_WARNINGS_FILE)) {
          return new File(o[1]);
        }
      }
      return null;
    ...
    private void addRelatedClasses(ClassDoc type) {
        ...
        for (SeeTag t: type.seeTags()) {
            ...
            if (a.compareTo(b) != 0 && !isConnected(type, t.referencedClass())) {
                if (a.compareTo(b) < 0) {
                    addEdge(new Edge(
                            root, SEE_ALSO, type,
                            b + " - - &#171;see also&#187;"));
    ...
    private boolean isConnected(ClassDoc c1, ClassDoc c2)
    {
      if (isConnected(c1, c2, edges)) {
        return true;
      }
      
      if (isConnected(c1, c2, reversedEdges)) {
        return true;
      }
    ...
    private boolean isConnected(ClassDoc c1, ClassDoc c2, Map<ClassDoc, Set<Edge>> map)
    {
      Set<Edge> set = map.get(c1);
      if (set != null) {
        for (Edge edge : set)
        {
          if (edge.getTarget() == c2) {
            return true;
          }
        }
      }
    ...
    public String getOverviewSummaryDiagram(JDepend jdepend) {
        ...
        int prefixLen = getLeastCommonPrefix(packages.keySet());
    ...
    private int getLeastCommonPrefix(Set<String> packages)
    {
      ...
      int prefixLen = 0;
      if (!packages.isEmpty()) {
          String firstPackageName = packages.iterator().next();
    ...
    public String getGroupOverviewDiagram() {
        Map<String, PackageDoc> packages = new TreeMap<String, PackageDoc>(new Comparator<String>() {
            public int compare(String o1, String o2) {
                return o2.compareTo(o1);
            }
        ...
        addGroupDependencies(packages, edgesToRender);
...
//        }
    ...
    private void addGroupDependencies(Map<String, PackageDoc> packages, Set<Edge> edgesToRender) {
//        Map<String, PackageDoc> allPackages = APIviz.getPackages(root);
//        PackageDoc[] packagesToGroup = allPackages.values().toArray(new PackageDoc[allPackages.size()]);
//
//        com.sun.tools.doclets.internal.toolkit.util.Group group = 
//          com.sun.tools.doclets.standard.Standard.htmlDoclet.configuration().group;
//        Map<String, List<String>> groupPackages = group.groupPackages(packagesToGroup);
//        
//        for (String pname: allPackages.keySet()) {
//            packages.put(pname, allPackages.get(pname));
//    
...
//        }
    ...
    public String getPackageSummaryDiagram(PackageDoc pkg) {
        ...
        boolean forceNosubtypes = !shouldShowSubtypes(root.options());
        for (ClassDoc node: nodes.values()) {
            fetchSubgraph(pkg, node, nodesToRender, edgesToRender, true, false, true, forceNosubtypes);
        }
    ...
    private void fetchSubgraph(
            ...
            boolean useHidden, boolean useSee, boolean forceInherit, boolean forceNosubtypes) {
        ...
        if (cls.containingPackage() == pkg) {
            ...
            if (reversedDirectEdges != null) {
                for (Edge edge: reversedDirectEdges) {
                    ...
                    if ((cls.tags(TAG_EXCLUDE_SUBTYPES).length > 0 || forceNosubtypes) &&
    ...
    private static boolean shouldShowSubtypes(String[][] options) {
      for (String[] o: options) {
        if (o[0].equals(OPTION_NO_SUBTYPES)) {
          return false;
        }
      }
      return true;
    ...
    public String getClassDiagram(ClassDoc cls) {
        ...
        fetchSubgraph(pkg, cls, nodesToRender, edgesToRender, false, true, false, false);
