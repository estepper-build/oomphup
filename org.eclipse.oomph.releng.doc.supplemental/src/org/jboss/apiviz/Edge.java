 ...
 *
 * @version $Rev: 78 $, $Date: 2010-03-04 07:37:22 +0100 (Do, 04 Mrz 2010) $
 *
 */
public class Edge implements Comparable<Edge> {
    ...
    public Edge(final RootDoc rootDoc, EdgeType type, Doc source, String spec) {
        ...
        if (spec.startsWith("{@link ")) {
          ...
          spec = spec.substring(7, end).trim() + " " + spec.substring(end + 1).trim();
        }
        ...
        final String label;
        if (args.length == 1) {
            target = getTarget(rootDoc, source, args);
            sourceLabel = "";
            targetLabel = "";
            label = "";
            oneway = true;
        ...
        if (type == EdgeType.DEPENDENCY && label.length() != 0) {
          edgeLabel = "&#171;" + label + "&#187;";
        } else {
          edgeLabel = label;
        }
    ...
    private Doc getTarget(RootDoc rootDoc, Doc source, final String[] args) {
      Doc target = rootDoc.classNamed(args[0]);
      if (target == null && source instanceof ClassDoc) {
        ClassDoc classDoc = (ClassDoc)source;
        String packageName = classDoc.containingPackage().name();
        String className = packageName + "." + args[0];
        target = rootDoc.classNamed(className);
        if (target !=null) {
          args[0] = className;
        } 
      }
